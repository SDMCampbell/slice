...
import { gamingContent } from '../data/gamingContent';
...
export function GamingPage() {
  ...
  const categories = useMemo(() => {
    const uniqueCategories = Array.from(new Set(gamingContent.map(item => item.category)));
    return ['All', ...uniqueCategories];
  }, []);
  
  // Filter items based on active category
  const filteredItems = useMemo(() => {
    if (activeCategory === 'All') return gamingContent;
    return gamingContent.filter(item => item.category === activeCategory);
  }, [activeCategory]);
